...
//specialized for Linux and Windows for performance - it will still work on anything that only provides libc
#if defined(linux)
#include <sys/stat.h>
...
int main(int argc, char * argv[])
{
	try
	{
		...
		if (verbose)
		{
			puts(S"Asar "+dec(asarver_maj)+"."+dec(asarver_min)+((asarver_bug>=10 || asarver_min>=10)?".":"")+dec(asarver_bug)+", by Alcaro");
		}
		string asmname=libcon_require_filename("Enter patch name: ");
		string romname=libcon_optional_filename("Enter ROM name: ", NULL);
#ifdef _WIN32
		if (asmname[0]=='"' && asmname[2]==':') asmname=substr(asmname+1, strlen(asmname+1)-1);
		if (romname[0]=='"' && romname[2]==':') romname=substr(romname+1, strlen(romname+1)-1);
#endif
		...
		if (romlen>=32768 && !ignoreerrors)
		{
			...
			if ((romdata[snestopc(0x00FFDE)]^romdata[snestopc(0x00FFDC)])!=0xFF ||
					(romdata[snestopc(0x00FFDF)]^romdata[snestopc(0x00FFDD)])!=0xFF) score-=42;//checksum doesn't match up to 0xFFFF? Very unlikely to be a ROM.
		...
		else if (verbose) puts("Assembling completed without problems.");
		closerom();
		reseteverything();
	}
	catch(errfatal&)
